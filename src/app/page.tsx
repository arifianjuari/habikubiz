export default function Home() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F9FAFB] font-sans text-[#111827]">
      <div className="mx-auto w-full max-w-[1440px] bg-white">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur md:px-10 lg:px-16">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <a href="/login" className="hidden min-h-11 rounded-full px-5 text-sm font-medium text-[#4F46E5] md:flex md:items-center">Masuk Ortu</a>
              <a href="/login" className="inline-flex min-h-11 items-center rounded-full bg-[#4F46E5] px-5 text-sm font-medium text-white shadow-lg shadow-indigo-200">
                Mulai Sekarang
              </a>
            </div>
          </div>
        ...
        <section className="grid gap-10 px-5 pb-12 pt-10 md:px-10 lg:grid-cols-[0.94fr_1.06fr] lg:px-16 lg:pb-20 lg:pt-16">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href="/login" className="inline-flex min-h-12 items-center justify-center rounded-full bg-[#4F46E5] px-7 text-base font-semibold text-white shadow-xl shadow-indigo-200">
                Mulai Buat Profil Anak
              </a>
              <a href="#alur" className="inline-flex min-h-12 items-center justify-center rounded-full border border-slate-300 bg-white px-7 text-base font-semibold text-[#111827]">
                Lihat Alur App
              </a>
            </div>
              ...
              <div className="rounded-[22px] bg-[#F9FAFB] p-4">
                ...
                <div className="mt-5 grid grid-cols-5 gap-2 text-center text-[10px] font-medium text-[#6B7280]">
                  {['Br', 'Ks', 'Pr', 'Lp', 'Pf'].map((item) => (
                    <div key={item} className="rounded-xl bg-white py-2">{item}</div>
                  ))}
                </div>
              </div>
            </div>
        ...
        <section id="fitur" className="bg-[#F9FAFB] px-5 py-14 md:px-10 lg:px-16">
          ...
          <div className="mt-9 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {features.map((feature) => (
              <article key={feature.title} className={`rounded-2xl border p-6 ${feature.tone}`}>
        ...
        <section id="parent" className="grid gap-8 bg-indigo-50 px-5 py-14 md:px-10 lg:grid-cols-[0.9fr_1.1fr] lg:px-16">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              {modules.map((module) => (
                <span key={module} className="rounded-full bg-white px-4 py-2 text-sm font-medium text-[#4F46E5] shadow-sm">{module}</span>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[32px] bg-[#111827] p-8 text-white md:p-12">
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
              ...
              <div className="flex flex-col gap-3 sm:flex-row lg:justify-end">
                <a href="/login" className="inline-flex min-h-12 items-center justify-center rounded-full bg-white px-7 font-semibold text-[#111827]">Daftar Sebagai Orang Tua</a>
                <a href="#fitur" className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/30 px-7 font-semibold text-white">Pelajari Modul</a>
              </div>
            </div>
          </div>
        </section>
...
function ParentRow({ child, biz, points, initials }: { child: string; biz: string; points: string; initials: string }) {
  return (
    <div className="flex items-center justify-between rounded-2xl border border-slate-200 p-4">
      <div className="flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-amber-100 text-sm font-semibold text-amber-800">{initials}</div>
        <div>
